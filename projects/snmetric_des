...
def desanal(file):
    ...
    surveymetrics.snmetric.Metric.prune_by_fieldname(obs,desfields)

    # do one field at a time
    for f in desfields:
        obs_f=surveymetrics.snmetric.Metric.get_by_fieldname(obs, f)
        if f[-1]=='3':
        ...
        for b in bands:
            ...
            metric = surveymetrics.snmetric.Metric.OneFieldBandMetric(omega,zmax,snr0,T0, tau, dt0,a)
          
            obs_0=surveymetrics.snmetric.Metric.get_by_band(obs_f,b)
 
            # observations into nights
            obs_0 = surveymetrics.snmetric.Metric.group_by_night(obs_0,longitude)
...
#./snmetric_des  ../data/schedule15/obssim-19/obs.txt
