...
class TestMetric(unittest.TestCase):
    ...
    def test_read_astac(self):
        ...
        obs=surveymetrics.snmetric.Metric.read_astac(file)
    ...
    def test_prune_by_fieldname(self):
        ...
        obs=surveymetrics.snmetric.Metric.read_astac(file)
        surveymetrics.snmetric.Metric.prune_by_fieldname(obs,self.desfields)
        self.assertEquals(len(obs), 3206)
        obs = surveymetrics.snmetric.Metric.get_by_band(obs,'i')
        self.assertEquals(len(obs),626)
        obs = surveymetrics.snmetric.Metric.get_by_fieldname(obs,'SN-X3')
        self.assertEquals(len(obs),162)
        groups=surveymetrics.snmetric.Metric.group_by_night(obs,self.longitude)
    ...
    def test_onefieldband(self):
        ...
        onemetric= surveymetrics.snmetric.Metric.OneFieldBandMetric(omega,zmax,snr_0, t0, tau, dt0, a)
    ...
    def test_onefieldband_plot(self):
        ...
        onemetric= surveymetrics.snmetric.Metric.OneFieldBandMetric(1.,1.,snr_0, 30, tau, dt0,1)
        metric_1 = onemetric.metric(ston1,epoch1)
